...
objfun0 <- function(X, maxT, X0, Xss, p = para) {
  ...
  for(t in 1:maxT) {
    ...
    #ret[t, 2] <- -m[t] + p$b*C[t]
    ret[t, 2] <- p$b/m[t] - 1/C[t] + p$beta/(1+ppi[t+1])/C[t+1]
    ret[t, 3] <- -1 + (p$gamma+1)*p$mu*(C[t]/A[t])^(p$gamma+1)
    #ret[t, 4] <- -(1+ppi[t+1])*m[t+1] + (1+psi[t+1])*m[t]
    if (t > 1) {
      ret[t, 4] <- -(1+ppi[t])*m[t] + (1+psi[t])*m[t-1]
    } else {
      ret[t, 4] <- -(1+ppi[t])*m[t] + (1+psi[t])*X0$m
    }
    ret[t, 5] <- -log(A[t+1]) + p$rho_A*log(A[t])
    ret[t, 6] <- -(psi[t+1]-p$psi_ss) + p$rho_psi*(psi[t]-p$psi_ss)
  }
  return(c(ret))
}
...
Xinit <- c(rep(ss$C,maxT), rep(ss$ii,maxT), rep(ss$A,maxT), 
           rep(ss$ppi,maxT), rep(ss$m,maxT), rep(ss$psi,maxT))

Ainit1 <- 1.05
psiinit1 <- psi_ss
X0set1 <- list(A = Ainit1, psi = psiinit1, m = ss$m)
...
Ainit2 <- 1.0
psiinit2 <- -0.01
X0set2 <- list(A = Ainit2, psi = psiinit2, m = ss$m)
