#=======================================
# Chapter 7, Optimal MP (commitment)
...
objfun0 <- function(X, maxT, X0,p = para) {
  ...
  for(t in 1:maxT) {
    ...
    if (t == 1) {
      ...
      ret[t, 3] <- ppi[t]+psi[t]-psi[t-1]
    }
    ret[t, 4] <- p$lambda*x[t]-p$kappa*psi[t]
  }
  return(c(ret))
}
...
rsltfun <- function(rslt, maxT, X0, p = para) {
  ...
  dev <- list(x = x,  ppi = ppi, psi = psi, ii = ii, e = e, r = r)
...
maxT  <- 150
Xinit <- c(rep(0, maxT), rep(0, maxT), rep(0, maxT), rep(0, maxT))
...
plot(cbind(0:maxT2, c(0, X1[[1]]$ppi[1:maxT2])), main = expression(pi[t]),
  xlab = "", ylab = "", typ = "l")
plot(cbind(0:maxT2, c(0, X1[[1]]$psi[1:maxT2])), main = expression(psi[t]),
  xlab = "", ylab = "", typ = "l")
plot(cbind(0:maxT2, c(0, X1[[1]]$e[1:maxT2])), main = expression(e[t]),
  xlab = "", ylab = "", typ = "l")

write.csv(X1[[1]], "nkm_commit.csv", row.names = F)
dev.copy2eps(file= figpath %+% "nkm_commit.eps")
